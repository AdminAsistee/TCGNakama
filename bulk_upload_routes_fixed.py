...
async def bulk_upload_appraise(
    ...
    for idx, image_file in enumerate(images):
        try:
            ...
            #         admin_token, card_number, card_name
            #     )
            #     if existing_product:
            #         exists = True
...
async def bulk_confirm(
    ...
    for card in selected_cards:
        try:
            ...
            # INVENTORY INCREMENT DISABLED — always create new product
            ...
            #         results.append({"success": False, "card_name": card_name, "error": f"Failed to update inventory: {str(inv_error)}"})
            # else:
